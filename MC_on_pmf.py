...
def MakeGrid(system, num_bins=None):
  if not num_bins:
    num_bins = [cv.num_bins for cv in system.cv_list]
  xl = []
...
class Bond():
  ...
  def Other(self, node):
    if self.first == node:
      return self.second
    elif self.second == node:
      return self.first
    else:
      raise(IOError("could not find node {0} in bond {1}".format(node, self)))
...
class Network():
  ...
  def FindNode(self, cv_values):
    for n in self.nodes:
      if tuple(n.cv_values) == tuple(cv_values):
        return n
    return None
  ...
  def InitializeFromSystem(self, system, num_bins=None, max_E=None):
    ...
    for i in range(len(grid)):
      if pmf_grid.values[i] < max_E:
        self.AddNode(grid[i], pmf_grid.values[i], [D.values[i]
                                                   for D in D_grids])
    for i, node1 in enumerate(self.nodes):
      for node2 in self.nodes[i + 1:]:
        v = npy.abs(npy.array(node1.cv_values) - npy.array(node2.cv_values))
        if all(v <= 1.1 * step_sizes) and npy.sum(v <= 0.1 * step_sizes) == 1:
          self.LinkNodes(node1, node2, system.temperature)

  def BrownianDynamics(self, init_pos, nsteps=1000, final_nodes=[]):
    from numpy.random import choice
    n0 = self.FindNode(init_pos)
    if not n0:
      n0 = self.FindClosestNode(init_pos)
    n = n0
    nl = [n0]
    tl = [0.0]
  ...
  def LinkNodes(self, node1, node2, temperature):
    ...
    d = npy.dot(v, v)**0.5
    c = npy.abs(v) / (2. * d)
    D = npy.dot(c, node1.diffusion_constants) + \
        npy.dot(c, node2.diffusion_constants)
    kf = D / (d * d) * npy.exp(-(node2.energy -
                                 node1.energy) / (kB * temperature))
    kb = D / (d * d) * npy.exp(-(node1.energy -
                                 node2.energy) / (kB * temperature))
...
def GenerateStateNetworkFromPMF(system, step_sizes, max_E=None):
  if not len(step_sizes) == system.dimensionality:
    raise(IOError(
        "step_sizes should contain one step size for each dimensionality of the system"))
  if not max_E:
    max_E = system.max_E_plot
  network = Network()
...
def _brownian_dynamics(div_pot, x0=0.0, T=300.0, D=1.0, time_step=0.01, nsteps=1000):
  ...
  for i in range(nsteps):
    ...
    v = -div_pot(x) * D + (2 * D)**0.5 * \
        random.gauss(mu, sigma) / (time_step**0.5)
    x = x + v * time_step
    xl.append(x)
...
def PlotTrajOnPMF(system, traj, outputdir, filename, skip=100, n_levels=None, max_E=None):
  ...
  if system.pmf.dimensionality == 2:
    ...
    num_pads = max([system.pmf.cv_list[0].num_pads,
                    system.pmf.cv_list[1].num_pads])
    ...
    if system.pmf.cv_list[0].units:
      plt.xlabel("{0} [{1}]".format(
          system.pmf.cv_list[0].name, system.pmf.cv_list[0].units))
    else:
      plt.xlabel("{0}".format(system.pmf.cv_list[0].name))
    if system.pmf.cv_list[1].units:
      plt.ylabel("{0} [{1}]".format(
          system.pmf.cv_list[1].name, system.pmf.cv_list[1].units))
    else:
      plt.ylabel("{0}".format(system.pmf.cv_list[1].name))
    plt.plot([el.cv_values[0] for el in traj[::skip]], [el.cv_values[1]
                                                        for el in traj[::skip]], color="m")
